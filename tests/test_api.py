"""Unit tests for the scripting API (pvr.scene / pvr.plot / pvr.show)."""
import importlib
...
class TestPlotFunction:
    ...
    def test_plot_registers_data_in_axes(self):
        x = np.array([0.0, 1.0])
        PlotVR.plot(x, x, x, color='#f00')
        axes = PlotVR.scene(num=1).gcf().gca()
